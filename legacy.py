...
def patch_theano_gan(state):
    ...
    if func.startswith("G"):
        vars += layer(
            "4x4/Dense", relu / 4, flatten2(next(param_iter).transpose(1, 0, 2, 3))
        )
        vars += layer(
            "4x4/Conv", relu, next(param_iter).transpose(2, 3, 1, 0)[::-1, ::-1]
        )
        for res in range(3, resolution_log2 + 1):
            vars += layer(
                "%dx%d/Conv0" % (2 ** res, 2 ** res),
                relu,
                next(param_iter).transpose(2, 3, 1, 0)[::-1, ::-1],
            )
            vars += layer(
                "%dx%d/Conv1" % (2 ** res, 2 ** res),
                relu,
                next(param_iter).transpose(2, 3, 1, 0)[::-1, ::-1],
            )
        for lod in range(0, resolution_log2 - 1):
            vars += layer(
                "ToRGB_lod%d" % lod, linear, next(param_iter)[np.newaxis, np.newaxis]
            )
    ...
    if func.startswith("D"):
        ...
        for res in range(resolution_log2, 2, -1):
            ...
            vars += layer(
                "FromRGB_lod%d" % (resolution_log2 - (res - 1)),
                relu,
                next(param_iter)[np.newaxis, np.newaxis],
            )
        vars += layer(
            "4x4/Conv", relu, next(param_iter).transpose(2, 3, 1, 0)[::-1, ::-1]
        )
        vars += layer(
            "4x4/Dense0", relu, flatten2(next(param_iter)[:, :, ::-1, ::-1]).transpose()
        )
