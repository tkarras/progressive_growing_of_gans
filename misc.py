...
def load_network_pkl(run_id_or_result_subdir_or_network_pkl, snapshot=None):
    return load_pkl(
        locate_network_pkl(run_id_or_result_subdir_or_network_pkl, snapshot)
    )
...
def draw_text_label(
    ...
    **kwargs
    ...
    xx, yy = (
        int(np.rint(x - alpha.shape[1] * alignx)),
        int(np.rint(y - alpha.shape[0] * aligny)),
    )
    xb, yb = max(-xx, 0), max(-yy, 0)
    xe, ye = (
        min(alpha.shape[1], img.shape[1] - xx),
        min(alpha.shape[0], img.shape[0] - yy),
    )
    img = np.array(img)
    ...
    slice[:] = (
        ...
        * (
            ...
            - (
                1.0
                - (1.0 - alpha[yb:ye, xb:xe])
                * (1.0 - glow[yb:ye, xb:xe] * glow_opacity)
            )
            * opacity
        )[:, :, np.newaxis]
    )
    slice[:] = (
        slice
        + alpha[yb:ye, xb:xe, np.newaxis] * (color * opacity)[np.newaxis, np.newaxis, :]
    )
    return img
...
def setup_text_label(
    ...
    alpha = moviepy.editor.TextClip(text, font=font, fontsize=fontsize).mask.make_frame(
        0
    )
