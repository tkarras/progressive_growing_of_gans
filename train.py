...
class TrainingSchedule:
    def __init__(
        ...
        if self.resolution in max_minibatch_per_gpu:
            self.minibatch = min(
                self.minibatch, max_minibatch_per_gpu[self.resolution] * config.num_gpus
            )
...
def train_progressive_gan(
    ...
    training_set = dataset.load_dataset(
        data_dir=config.data_dir, verbose=True, **config.dataset
    )
    ...
    grid_fakes = Gs.run(
        grid_latents, grid_labels, minibatch_size=sched.minibatch // config.num_gpus
    )
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            if cur_tick % network_snapshot_ticks == 0 or done:
                misc.save_pkl(
                    (G, D, Gs),
                    os.path.join(
                        result_subdir, "network-snapshot-%06d.pkl" % (cur_nimg // 1000)
                    ),
                )
