...
def autosummary(name, value):
    ...
    if is_tf_expression(value):
        ...
        if name not in _autosummary_immediate:
            with absolute_name_scope("Autosummary/" + id), tf.device(
                None
...
class Optimizer:
    ...
    def register_gradients(self, loss, vars):
        ...
        assert all(
            shape_to_list(var.shape) == var_shape
            for var, var_shape in zip(vars, self._grad_shapes)
        )
        dev = loss.device
                    ...
                    if dev == devices[-1]:
                        with tf.name_scope("Statistics"):
                            ...
                            if self.use_loss_scaling:
                                ops.append(
                                    autosummary(self.id + "/loss_scaling_log2", ls_var)
                                )
...
class Network:
    ...
    def _init_fields(self):
        ...
        self.scope = (
            None
        )  # Unique TF graph scope, derived from the user-specified name.
    ...
    def _init_graph(self):
        ...
        self.scope = tf.get_default_graph().unique_name(
            self.name.replace("/", "_"), mark_as_used=False
        )
        ...
        self.output_templates = (
            [out_expr] if is_tf_expression(out_expr) else list(out_expr)
        )
        self.output_names = [
    ...
    def get_var_localname(self, var_or_globalname):
        ...
        globalname = (
            var_or_globalname
            if isinstance(var_or_globalname, str)
            else var_or_globalname.name
        )
    ...
    def find_var(self, var_or_localname):
        ...
        return (
            self.vars[var_or_localname]
            if isinstance(var_or_localname, str)
            else var_or_localname
        )
    ...
    def __setstate__(self, state):
        ...
        module = imp.new_module(
            "_tfutil_network_import_module_%d" % len(_network_import_modules)
        )
    ...
    def copy_trainables_from(self, src_net):
        assert isinstance(src_net, Network)
        name_to_value = run(
            {name: src_net.find_var(name) for name in self.trainables.keys()}
        )
        set_vars({self.find_var(name): value for name, value in name_to_value.items()})
            ...
            with tf.name_scope("MovingAvg"):
                ...
                for name, var in self.vars.items():
                    if name in src_net.vars:
                        cur_beta = (
                            beta if name in self.trainables else beta_nontrainable
                        )
    ...
    def run(
        ...
        **dynamic_kwargs
        ...
        key = str(
            [
                list(sorted(dynamic_kwargs.items())),
                num_gpus,
                out_mul,
                out_add,
                out_shrink,
        ...
        if key not in self._run_cache:
            with absolute_name_scope(self.scope + "/Run"), tf.control_dependencies(
                None
            ):
                in_split = list(
                    zip(*[tf.split(x, num_gpus) for x in self.input_templates])
                )
                out_split = []
                for gpu in range(num_gpus):
                    with tf.device("/gpu:%d" % gpu):
                        out_expr = self.get_output_for(
                            *in_split[gpu], return_as_list=True, **dynamic_kwargs
                        )
                        if out_mul != 1.0:
                        ...
                        if out_dtype is not None:
                            ...
                            out_expr = [
                                tf.saturate_cast(x, out_dtype) for x in out_expr
                            ]
        ...
        out_arrays = [
            np.empty([num_items] + shape_to_list(expr.shape)[1:], expr.dtype.name)
    ...
    def list_layers(self):
        ...
        all_ops = [
            op for op in all_ops if not any(p in op.name for p in patterns_to_ignore)
        ]
        layers = []
