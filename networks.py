...
def get_weight(shape, gain=np.sqrt(2), use_wscale=False, fan_in=None):
    ...
    if use_wscale:
        ...
        return (
            tf.get_variable(
                "weight", shape=shape, initializer=tf.initializers.random_normal()
            )
            * wscale
        )
    else:
...
def conv2d(x, fmaps, kernel, gain=np.sqrt(2), use_wscale=False):
    ...
    w = get_weight(
        [kernel, kernel, x.shape[1].value, fmaps], gain=gain, use_wscale=use_wscale
    )
    w = tf.cast(w, x.dtype)
    ...
    **kwargs
    ...
    def fromrgb(x, res):  # res = 2..resolution_log2
        with tf.variable_scope("FromRGB_lod%d" % (resolution_log2 - res)):
            return act(
                apply_bias(
                    conv2d(x, fmaps=nf(res - 1), kernel=1, use_wscale=use_wscale)
                )
            )
        ...
        with tf.variable_scope("%dx%d" % (2 ** res, 2 ** res)):
            if res >= 3:  # 8x8 and up
                ...
                with tf.variable_scope("Dense0"):
                    x = act(
                        apply_bias(dense(x, fmaps=nf(res - 2), use_wscale=use_wscale))
                    )
                with tf.variable_scope("Dense1"):
                    x = apply_bias(
                        dense(x, fmaps=1 + label_size, gain=1, use_wscale=use_wscale)
                    )
            return x
