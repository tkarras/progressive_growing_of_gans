    ...
    with open(os.path.join(src_result_subdir, "log.txt"), "rt") as log:
        for line in log:
            ...
            if k and l and t:
                ...
                png = os.path.join(
                    src_result_subdir, "fakes%06d.png" % int(np.floor(k))
                )
        ...
        if png_cache[0] == png:
            ...
            while img.shape[1] > 1920 or img.shape[0] > 1080:
                img = (
                    ...
                    .reshape(img.shape[0] // 2, 2, img.shape[1] // 2, 2, -1)
                    .mean(axis=(1, 3))
                )
            png_cache[:] = [png, img]
        img = misc.draw_text_label(
            img, "lod %.2f" % lod, 16, img.shape[0] - 4, alignx=0.0, aligny=1.0
        )
        img = misc.draw_text_label(
            img,
...
def evaluate_metrics(
    run_id, log, metrics, num_images, real_passes, minibatch_size=None
):
    metric_class_names = {
    ...
    for name in metrics:
        ...
        for idx in range(10):
            obj.feed(
                mode,
                np.random.randint(
                    0, 256, size=[minibatch_size] + image_shape, dtype=np.uint8
                ),
            )
        obj.end(mode)
