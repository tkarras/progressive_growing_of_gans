...
def create_celebahq(tfrecord_dir, celeba_dir, delta_dir, num_threads=4, num_tasks=100):
    ...
    expected_images = 202599
    if (
        len(glob.glob(os.path.join(celeba_dir, "img_celeba", "*.jpg")))
        != expected_images
    ):
        error("Expected to find %d images" % expected_images)
    with open(
        os.path.join(celeba_dir, "Anno", "list_landmarks_celeba.txt"), "rt"
    ) as file:
        landmarks = [
    ...
    if getattr(PIL, "PILLOW_VERSION", "") != "3.1.1":
        error(
            "create_celebahq requires pillow version 3.1.1"
    ...
    def process_func(idx):
        ...
        if max(pad) > border - 4:
            ...
            mask = 1.0 - np.minimum(
                ...
                np.minimum(
                    np.float32(w - 1 - x) / pad[2], np.float32(h - 1 - y) / pad[3]
                ),
            )
            blur = 1024 * 0.02 / zoom
            img += (
        ...
        with zipfile.ZipFile(
            os.path.join(delta_dir, "deltas%05d.zip" % (idx - idx % 1000)), "r"
        ) as zip:
        ...
        delta = np.frombuffer(
            bz2.decompress(cryptography.fernet.Fernet(key).decrypt(delta_bytes)),
            dtype=np.uint8,
        ).reshape(3, 1024, 1024)
